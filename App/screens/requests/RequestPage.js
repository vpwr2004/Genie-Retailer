...
const RequestPage = () => {
  ...
  const requestInfo= useSelector(state => state.requestData.requestInfo || {});
    // console.log("params",JSON.parse(route.params.data.requestInfo));
      ...
      // //     dispatch(setRequestInfo(JSON.parse(route.params?.data?.requestInfo)));
      // //     setTimeout(()=>{
      ...
      const fetchRequestData = async () => {
        try {
          ...
          console.log("User data found successfully");
           ...
           console.log("requestInfo page", requestInfo?.id);
           let req;
           if(route?.params?.data){
            req=JSON.parse(route?.params?.data?.requestInfo);
           }
    ...
    <SafeAreaView style={{ flex: 1 }}>
      <View className="relative flex-grow">
        ...
        <View className="px-[50px] pb-[20px] flex bg-[#ffe7c8]">
          ...
          <Text>{requestInfo?.requestId?.requestDescription}</Text>
          {/* {
            ...
            { messages && messages?.map((message) => {
              ...
              if(message?.bidType==="update"){
                return (
                  ...
                  >
                    <Text className="text-[16px] text-[#FB8C00]">{message?.message}</Text>
                  </View>
                );
